...
It is the canonical current prompt-delivery contract page. It is contrast-only and does not redefine the redesign target prompt canon.
...
This page owns the current shipped prompt source map only. It does not define the redesign target prompt canon.
