...
- inspect task compose state, bound services, and derived runtime/session mounts or logs without scraping transcripts
...
- task compose, derived runtime/session state, typed events, and logs are inspectable without turning them into the control truth
