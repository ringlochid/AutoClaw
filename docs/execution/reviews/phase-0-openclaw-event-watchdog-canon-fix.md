...
- pass/fail: pass
- summary: the stronger OpenClaw event-handling and watchdog timing contract is now locked in canon without falsely claiming the code has already landed the redesign. The 4A owner docs pin the raw-envelope versus normalization split, the 4B owner docs move stale-timeout anchoring away from checkpoint time, the config owner page adopts first-progress wording, and the current-contrast pages stay truthful about the shipped gap.
