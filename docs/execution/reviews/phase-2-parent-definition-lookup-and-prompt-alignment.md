...
- inspected runtime/docs/tests: shipped prompt block assets, touched prompt-layer owner docs and generated examples, `apps/api/app/runtime/prompt/instructions.py`, `apps/api/app/runtime/prompt/sections/rendering.py`, and `apps/api/tests/unit/runtime_prompt_rendering/test_dispatch.py`
...
- owned-surface compliance: the Phase 2 artifact list stayed inside `docs/redesign/prompt-layer/**`, `apps/api/app/runtime/prompt/**`, `apps/api/tests/unit/runtime_prompt_rendering/**`, and the selected execution artifacts
...
- exact repo search: `rg -n "^def _|^class _|from .* import _|import _" apps/api/app/runtime/prompt/instructions.py apps/api/app/runtime/prompt/sections/rendering.py apps/api/tests/unit/runtime_prompt_rendering/test_dispatch.py`
- retained module-local private helpers: `_full_prompt_instruction_block_ids`, `_instruction_block_ids`, and `_render_node_guidance_block` in `apps/api/app/runtime/prompt/instructions.py`
- cross-module shared-helper search: `rg -n "_render_node_guidance_block|_full_prompt_instruction_block_ids|_instruction_block_ids|CURRENT_ONLY_DEFINITION_LOOKUP_GUIDANCE|DEFINITION_REVISION_HISTORY_EXCLUSION_GUIDANCE" apps/api/app/runtime/prompt apps/api/tests/unit/runtime_prompt_rendering`
...
- oversized-function inventory: longest touched function is `render_allowed_actions_now()` at lines 179-245 in `apps/api/app/runtime/prompt/sections/rendering.py`; no touched function exceeds the 80-line trigger

## Stale-logic search proof

- search commands:
  - `rg -n "search_definitions|get_definition|revision history|palette-first|current-only" docs/redesign/prompt-layer apps/api/app/runtime/prompt apps/api/tests/unit/runtime_prompt_rendering/test_dispatch.py docs/redesign/workflows/parent-root-planning-surface.md docs/current/interfaces/current-definition-bootstrap-and-task-upload.md`
  - `rg -n "registry read lane|definition registry/tool read surface|list_definition_versions|use only role and policy names from the surfaced structural edit palette|role and policy names must come only from the surfaced structural edit palette|role/policy names only from the surfaced structural edit palette" docs/redesign/prompt-layer apps/api/app/runtime/prompt apps/api/tests/unit/runtime_prompt_rendering/test_dispatch.py`
...
- exact search: `rg -n "task compose as a runtime-derived kitchen sink|redesign docs treated as the shipped prompt source|prompt rules that rely on hidden transcript memory|filesystem-primary truth for generated roots|runtime persistence truth split across both Phase 2 and Phase 3" docs/redesign/prompt-layer apps/api/app/runtime/prompt docs/execution/plans/phase-2-parent-definition-lookup-and-prompt-alignment.md docs/execution/evidence/phase-2-parent-definition-lookup-and-prompt-alignment.md`
...
- redesign owners relied on: `docs/redesign/prompt-layer/contract.md`, `docs/redesign/prompt-layer/source-and-sections.md`, `docs/redesign/prompt-layer/field-renderers.md`, `docs/redesign/prompt-layer/prompt-pack/system-and-provider-block.md`, `docs/redesign/prompt-layer/prompt-pack/runtime-rule-blocks.md`, and `docs/redesign/prompt-layer/prompt-resource-usage-appendix.md`
- supporting redesign reads or appendix owners relied on: `docs/redesign/prompt-layer/README.md`, `docs/redesign/prompt-layer/INDEX.md`, `docs/redesign/prompt-layer/generated/rendered-examples.md`, `docs/redesign/prompt-layer/composition-example.md`, and `docs/redesign/workflows/parent-root-planning-surface.md`
- current-contrast pages relied on: `docs/current/interfaces/prompt-layer-and-worker-delivery.md` and `docs/current/interfaces/current-definition-bootstrap-and-task-upload.md`
...
- code or tests inspected: `apps/api/app/runtime/prompt/instructions.py`, `apps/api/app/runtime/prompt/sections/rendering.py`, `apps/api/tests/unit/runtime_prompt_rendering/test_dispatch.py`, and the shipped prompt block assets under `apps/api/app/runtime/prompt/assets/blocks/`
