...
- `P0-WP3`: patch the file lock map and redesign-code-landing map so execution routing, proof gates, and later closeout artifacts line up with the same ownership split
...
4. Patch `docs/execution/maps/file-priority-map.md` and `docs/execution/maps/redesign-code-landing-map.md` so their ownership, collateral, and proof wording matches the same split.
...
- stop if truthful completion requires `docs/current/**` edits beyond direct replacement-link repair
- stop if truthful completion requires `docs/redesign/**` edits instead of execution-canon fixes
