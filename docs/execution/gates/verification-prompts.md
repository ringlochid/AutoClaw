...
5. Confirm the landed work stayed within the locked implementation surfaces, or that any phase-bounded re-scope or canon patch was recorded explicitly.
6. List the primary code surfaces, docs, prompts, gates, and examples that should have changed.
7. Confirm whether they changed enough to land the new design.
8. Confirm whether the shared TDD rule was followed for behavior changes, or whether an exact exception was recorded.
9. List the repo-native quality gates that should have run for the touched surfaces.
10. Confirm whether they ran and whether they passed.
11. List the unit, integration, currently-viable e2e, and any required SQLite, Postgres+Docker, package, or reset verification lanes that should exist.
12. Confirm whether they exist and whether they pass.
13. Confirm whether the required deliverables, milestones, work packages, required supporting redesign reads, required current-contrast reads, required examples/diagrams, and exit evidence from the approved phase plan were satisfied.
14. Run the code quality gate and mandatory phase review.
15. Run the reset gate if DB/schema/package/public-surface truth changed.
16. Search for stale logic and phase kill-list terms that should have been removed.
17. Confirm that any phase-local required checklist was completed.
18. Review delegation results:
...
20. If anything is incomplete, do not claim done. Produce an exact remaining-work list.
```
