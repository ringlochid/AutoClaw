...
This page gives the shortest high-level explanation of the redesign runtime model.

Start at [README.md](README.md) for the layer-first map.

## Core change

The redesign makes controller/DB state the only runtime truth owner and turns node work into explicit bounded assignments plus explicit boundary returns.
