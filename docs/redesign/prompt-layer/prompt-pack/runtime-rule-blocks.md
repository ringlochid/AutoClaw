...
For structural edits, use only role and policy names already surfaced in the current prompt or manifest; do not guess them from transcript memory.
Runtime validation and commit authority still live on the runtime side.
If you use `add_child`, `update_child`, or `remove_child`, reread the current manifest first, use only role/policy names already surfaced in the current prompt or manifest, wait for tool success, then reread the regenerated manifest before deciding whether one child assignment should be staged.
