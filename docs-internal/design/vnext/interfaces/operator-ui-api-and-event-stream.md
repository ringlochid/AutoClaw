...
- replay order is persisted controller event order for that task stream
...
- backfill returns all later persisted events in controller order
- controller order is task-stream order; dispatch-local or provider-local order may appear as secondary fields only
