...
Current shipped API routes still keep registry truth under `/definitions` and task start under `/tasks/start`, but the tree now also exposes backend-owned flat definition draft routes under `/authoring/*`.
...
- mounted operator MCP does not expose draft authoring tools; mutating draft authoring remains on the HTTP `/authoring` workbench API
