...
- command: `rg -n "MQ|distributed-safe|message-queue|local-tool-first|effect-kind|inline-versus-after-return|sync/async ownership" AGENTS.md docs-internal/execution/v1` outcome: only the intended local-tool-first routing and non-goal-note wording remained in the owned canon surfaces
