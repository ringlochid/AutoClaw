...
| runtime control services | `apps/api/app/runtime/*` includes approvals, callback bindings, checkpoints, packaging, replan, watchdog, and mixed truths | `delete now` | runtime behavior will be rebuilt from design canon | Phase 2-4B |
...
| compiler core | `apps/api/app/compiler/*` is structurally useful, but stale edges, extends, task defaults, and skill-ref logic survive | `delete now` | current compiler behavior cannot be trusted | Phase 1 |
