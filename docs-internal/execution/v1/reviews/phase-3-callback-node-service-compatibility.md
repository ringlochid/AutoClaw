...
3. Low: the `app.cli` compatibility surface remained dirty in the worktree, but the owned closure path no longer depends on underscore-private `app.cli` helper aliases because `runtime_support.py` now uses `cli.command_env(...)` on the owned path.
...
- phase kill-list source: `docs-internal/execution/v1/phases/phase-3-runtime-parent-review-and-replan.md`
...
- design owners relied on: `docs-internal/design/v1/architecture/runtime-boundary-and-controller-loop-contract.md` and `docs-internal/design/v1/workflows/parent-root-release-and-closure.md`
- supporting design reads or appendix owners relied on: none beyond the phase-required reads for this narrow repair
- current-contrast pages relied on: `docs-internal/current/v1/interfaces/api-surface-and-route-map.md` and `docs-internal/current/v1/interfaces/api-trust-lanes.md`
