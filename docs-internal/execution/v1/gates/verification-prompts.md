...
```text
Run the post-implementation review for the current design phase.

Do not expand scope. Verify the landed work only against the approved phase plan and the canonical docs.

Tasks:
1. Re-read AGENTS.md, STYLE.md, the approved phase plan from `docs-internal/execution/v1/plans/`, the current phase page, the implementation file lock map, and the primary design reference pages.
2. Re-read the required supporting design reads, required current-contrast pages, required examples, and required diagrams named by the current phase page.
...
14. Confirm whether the required deliverables, milestones, work packages, required supporting design reads, required current-contrast reads, required examples/diagrams, and exit evidence from the approved phase plan were satisfied and recorded under phase-scoped artifacts in `docs-internal/execution/v1/evidence/`.
...
- were required supporting design reads, required current-contrast pages, and required examples/diagrams from the current phase page actually used?
