...
The execution pack does not keep a separate repo-global active-phase marker. Pre-implementation review must select the current phase using the phase selection rule in `docs-internal/execution/v1/phases/overview.md` and name that phase page explicitly before planning starts.
...
- read the required supporting design reads named by the phase page when live target semantics, durable decisions, how-to guidance, or tutorial coverage matter
...
- common appendix owners include `docs-internal/design/v1/workflows/workflow-schema-appendix.md`, `docs-internal/design/v1/interfaces/api-schema-appendix.md`, and `docs-internal/design/v1/prompt-layer/prompt-resource-usage-appendix.md` when the selected phase points there
...
- use `docs-internal/execution/v1/maps/design-code-landing-map.md` when the selected phase must land target owners, examples, or proof gates in code
...
```text
All the phases descriptions are in docs-internal/execution/v1.

Run the pre-implementation review for the current design phase.
...
2. Re-read AGENTS.md, STYLE.md, the current phase page, the implementation
   file lock map, and the primary design references named by the phase page.
3. Re-read any required supporting design reads, required current-contrast
...
5. Use `docs-internal/execution/v1/maps/design-code-landing-map.md` when the blocker
...
The read list below is the mandatory minimum read set for the selected phase; add more canon, code, tests, examples, or diagrams when completeness requires it, but do not treat the named reads as optional. The selected current phase page plus `docs-internal/execution/v1/maps/file-priority-map.md` remain authoritative for phase-local requirements, reads, and owned surfaces. Do not treat implementation surfaces as suggestions; if the work needs edits outside them, stop and either patch canon first, re-scope the work package, or move the change to the owning phase.
...
- the primary design pages named by the phase page
...
- any named appendix owners when exact API/schema/prompt/payload detail matters,
  including `docs-internal/design/v1/workflows/workflow-schema-appendix.md`,
  `docs-internal/design/v1/interfaces/api-schema-appendix.md`, and
  `docs-internal/design/v1/prompt-layer/prompt-resource-usage-appendix.md` when the
  selected phase points there
- `docs-internal/execution/v1/maps/design-code-landing-map.md` when the phase must land
...
- name the plan artifact path under `docs-internal/execution/v1/plans/` and the matching evidence/review artifact paths under `docs-internal/execution/v1/evidence/` and `docs-internal/execution/v1/reviews/`
