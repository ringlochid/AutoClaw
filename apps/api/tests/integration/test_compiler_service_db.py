...
async def test_compile_default_workflow_persists_compiled_plan(
    ...
    await bootstrap_registry(
        db_session,
        publish=True,
        definitions_root=registry_service.DEFINITIONS_ROOT,
    )
    ...
    assert persisted_nodes[1].skill_bindings[0]["runtime_name"] == "autoclaw-contract-checker"
...
async def test_compile_max_complexity_review_matches_compact_seeded_graph(
    db_session: AsyncSession,
) -> None:
    await bootstrap_registry(
        db_session,
    ...
    compiled_plan = await compile_published_workflow(db_session, "max-complexity-review")
    await db_session.commit()

    assert [node.node_key for node in compiled_plan.nodes] == [
        "root",
        "root.discovery",
        "root.product",
    ...
    assert [
        (edge.from_node_key, edge.to_node_key, edge.edge_kind.value) for edge in compiled_plan.edges
    ] == [
        ("root", "root.discovery", "control"),
        ("root", "root.product", "control"),
        ("root", "root.implementation_loop", "control"),
        ("root.implementation_loop", "root.implementation_loop.cycle", "control"),
        ("root.discovery", "root.review_and_governance", "dependency"),
        ("root.product", "root.review_and_governance", "dependency"),
        ("root.implementation_loop.cycle", "root.review_and_governance", "dependency"),
        ("root.review_and_governance", "root.review_and_governance.security", "control"),
        ("root.review_and_governance.security", "root.sync", "control"),
    ]
