...
async def wait_for_path(path: Path, *, task_id: str, max_wait_seconds: float = 5.0) -> None:
    try:
        await drive_runtime_until(
            lambda: asyncio.to_thread(path.is_file),
            task_id=task_id,
            max_cycles=_runtime_wait_cycles(max_wait_seconds),
        )
    except AssertionError as exc:
        raise TimeoutError(
            f"expected materialized path '{path}' within {max_wait_seconds:.2f}s"
        ) from exc
...
async def wait_for_provider_event_payloads(
    ...
    *,
    ...
    async def provider_event_payloads_ready() -> bool:
        nonlocal event_payloads
        if not await asyncio.to_thread(path.is_file):
            return False
        event_payloads = load_provider_event_payloads(path)
        return bool(event_payloads)

    try:
        await drive_runtime_until(
            provider_event_payloads_ready,
            task_id=task_id,
            max_cycles=_runtime_wait_cycles(max_wait_seconds),
        )
...
async def wait_for_support_state_json(
    ...
    *,
    ...
    matching_payload: dict[str, object] | None = None

    async def support_state_ready() -> bool:
        nonlocal matching_payload
        if not await asyncio.to_thread(path.is_file):
            return False
        payload = cast(
            dict[str, object],
...
def _runtime_wait_cycles(max_wait_seconds: float) -> int:
    return max(1, int(max_wait_seconds * 20))
