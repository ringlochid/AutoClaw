...
def create_app(
    *,
    is_mcp_mounts_enabled: bool | None = None,
    **legacy_kwargs: Any,
) -> FastAPI:
    legacy_enable_mcp_mounts = legacy_kwargs.pop("enable_mcp_mounts", None)
    if legacy_kwargs:
        unexpected_keys = ", ".join(sorted(legacy_kwargs))
        raise TypeError(f"unexpected keyword arguments: {unexpected_keys}")

    if is_mcp_mounts_enabled is None and legacy_enable_mcp_mounts is not None:
        is_mcp_mounts_enabled = bool(legacy_enable_mcp_mounts)

    settings = get_settings()
    if is_mcp_mounts_enabled is None:
        is_mcp_mounts_enabled = settings.env != Environment.TEST
