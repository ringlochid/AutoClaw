...
from autoclaw.runtime.watchdog.task_rows import build_watchdog_context, load_watchdog_task_rows
...
async def _classify_task_dispatches(
    ...
    *,
    ...
    task_rows = await load_watchdog_task_rows(
        session,
        flow=flow,
        candidate_dispatch_ids=candidate_dispatch_ids,
    )
    for dispatch_id in candidate_dispatch_ids:
        context = build_watchdog_context(
            flow=flow,
