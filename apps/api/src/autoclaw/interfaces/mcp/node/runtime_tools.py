...
from autoclaw.runtime.contracts.parent_tools import ParentToolPayload
...
def register_node_runtime_tools(server: FastMCP) -> None:
    ...
    _register_remove_child_tool(server)
    _register_release_tools(server)
    _freeze_node_tool_contracts(server)


def _register_checkpoint_tools(server: FastMCP) -> None:
    @server.tool(
...
def _register_assign_child_tool(server: FastMCP) -> None:
    @server.tool(
        name="assign_child",
    ...
    async def assign_child_tool(
        ...
        return _build_node_success_tool_result(
            await _write_parent_tool_operation(
                task_id=task_id,
                session_key=session_key,
...
def _register_add_child_tool(server: FastMCP) -> None:
    @server.tool(
        name="add_child",
...
def _register_update_child_tool(server: FastMCP) -> None:
    @server.tool(
        name="update_child",
...
def _register_remove_child_tool(server: FastMCP) -> None:
    @server.tool(
        name="remove_child",
...
def _register_release_tools(server: FastMCP) -> None:
    @server.tool(
        name="release_green",
...
async def _write_parent_tool_operation(
    *,
    task_id: str,
    session_key: str,
...
async def _write_node_operation(
    *,
    task_id: str,
    session_key: str,
...
def _freeze_node_tool_contracts(server: FastMCP) -> None:
...
__all__ = ["register_node_runtime_tools"]
