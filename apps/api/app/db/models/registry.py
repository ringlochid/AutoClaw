...
class RoleVersion(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("role_definition_id", "version", name="uq_role_versions_definition_version"),)
...
class PolicyVersion(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("policy_definition_id", "version", name="uq_policy_versions_definition_version"),)
    ...
    version: Mapped[int] = mapped_column(nullable=False)
    status: Mapped[DefinitionVersionStatus] = mapped_column(
        build_str_enum(DefinitionVersionStatus, name="definition_version_status", create_type=False),
        default=DefinitionVersionStatus.DRAFT,
        nullable=False,
    )
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class WorkflowVersion(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("workflow_definition_id", "version", name="uq_workflow_versions_definition_version"),)
...
class SkillVersion(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("skill_registry_id", "version_label", name="uq_skill_versions_registry_version"),)
