...
def bootstrap_task_runtime(bootstrap_input: RuntimeBootstrapInput) -> RuntimeBootstrapResult:
    task_root_paths = resolve_task_root_paths(
        task_root=bootstrap_input.task_root,
        task_compose=bootstrap_input.task_compose,
    )
    ensure_task_root_layout(task_root_paths)

    criteria_paths = write_criteria_files(
        paths=task_root_paths,
    ...
    write_manifest_projection(paths=task_root_paths, manifest=manifest)
    write_assignment_projection(
        paths=task_root_paths,
        attempt_id=bootstrap_input.attempt_id,
        assignment=assignment,
    ...
    write_prompt_artifact(
        ...
        full_markdown=prompt_bundle.full_markdown,
    )
