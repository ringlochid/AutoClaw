...
from app.db.models.runtime.shared import utcnow
...
_RUNNER_BY_LOOP: dict[int, _RuntimeEffectRunnerState] = {}
...
@dataclass
class _RuntimeEffectRunnerState:
...
def _queued_effects(session: AsyncSession) -> list[_QueuedEffect]:
    return cast(list[_QueuedEffect], session.info.setdefault(_QUEUE_KEY, []))

def _seen_keys(session: AsyncSession) -> set[tuple[str, ...]]:
    return cast(set[tuple[str, ...]], session.info.setdefault(_SEEN_KEY, set()))

def runtime_effect_dedupe_key(key: tuple[str, ...]) -> str:
    return "|".join(key)

def _runtime_effect_id(key: tuple[str, ...]) -> str:
    ...
    *,
    ...
    _queued_effects(session).append(
        _QueuedEffect(
            ...
            payload=payload,
            priority=_EFFECT_PRIORITIES[effect_kind],
        )
    )
...
async def stage_post_commit_effects(session: AsyncSession) -> bool:
    ...
    for effect in queued:
        ...
        if row is None:
            session.add(
                RuntimeEffectModel(
                    runtime_effect_id=_runtime_effect_id(effect.key),
...
async def _drain_ready_effects(state: _RuntimeEffectRunnerState) -> bool:
    processed_any = False
...
async def _run_effect_runner(state: _RuntimeEffectRunnerState) -> None:
    try:
...
def _ensure_runner_started() -> _RuntimeEffectRunnerState:
    loop_id = _loop_id()
    ...
    idle = asyncio.Event()
    started = asyncio.Event()
    state = _RuntimeEffectRunnerState(
